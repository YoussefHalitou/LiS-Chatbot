...
const chatConcurrencyLimiter = new InMemoryConcurrencyLimiter(6)
...
export async function POST(req: NextRequest) {
    ...
    return NextResponse.json(
      { error: 'Rate limit exceeded. Please wait and try again.' },
      {
        ...
        headers: {
          ...
          'X-Concurrency-Limit': chatConcurrencyLimiter.limit.toString(),
        },
      }
    )
  }
  ...
  const concurrency = chatConcurrencyLimiter.acquire('chat')

  if (!concurrency.allowed) {
    return NextResponse.json(
      { error: 'Too many concurrent chat requests. Please wait and try again.' },
      {
        status: 429,
        headers: {
          ...baseHeaders,
