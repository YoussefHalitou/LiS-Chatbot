...
| Medium | Performance | Large, multi-hundred-line system prompt and non-streaming responses increase latency; caching is absent and only small per-route concurrency caps (chat/tts=6, stt=4) exist. | Slower responses and degraded experience under load, with limited protection against noisy-neighbor effects. | Trim/reuse prompt context, enable streaming, add caching, and monitor concurrency saturation. |
...
- Per-route concurrency caps (chat/tts=6, stt=4) now shed bursts with `429` responses, but there is still no caching, batching, or streaming to reduce latency.
...
- **Concurrency caps:** Fire multiple parallel requests to `/api/chat` or `/api/tts` (limit 6 each) and `/api/stt` (limit 4); once caps are hit, expect `429` with `X-Concurrency-Limit` and `X-Concurrency-Active` headers.
