...
export default function ChatInterface() {
  ...
  // Restart recording after audio finishes in voice-only mode
  useEffect(() => {
    if (voiceOnlyMode && audioRef.current) {
      const audio = audioRef.current
      const handleAudioEnd = () => {
        // Wait a bit before restarting recording
        setTimeout(() => {
  ...
  }, [voiceOnlyMode, isRecording, isLoading, isPlayingAudio])
  ...
  const startRecording = async () => {
    ...
    try {
      ...
      // If starting voice-only mode, set the mode
      if (!voiceOnlyMode && !isRecording) {
        // This will be set when user clicks the mic button in normal mode
        // For voice-only mode, we'll handle it differently
      }
    } catch (error: any) {
        ...
        <div className="bg-blue-600 border-t border-blue-700 px-3 py-6 sm:px-4 sm:py-6 safe-area-inset-bottom">
          <div className="max-w-3xl mx-auto">
            <div className="flex flex-col items-center gap-4">
              {isRecording ? (
                <>
                  <div className="w-20 h-20 sm:w-24 sm:h-24 rounded-full bg-red-500 flex items-center justify-center animate-pulse shadow-lg">
                ...
                <>
                  <div className="w-20 h-20 sm:w-24 sm:h-24 rounded-full bg-green-500 flex items-center justify-center animate-pulse">
